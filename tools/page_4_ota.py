...
def batch_ota_worker(port, baud, files_data, loop_count, wait_minutes, ota_state, timeout, delay):
        ...
        for file_name, bin_bytes, offset_val, slave_val, pre_cmds, post_cmds in files_data:
            ...
            if pre_cmds:
                ...
                for p_addr, p_vals in pre_cmds:
                    ota_state["logs"].append(f"▶️ [前置操作] 向站号 {slave_val} 地址 {p_addr} 写入: {p_vals}")
                    ...
                    time.sleep(0.3)
                ota_state["logs"].append(f"⏳ 前置操作完毕，等待 1.5 秒设备缓冲...")
                time.sleep(1.5)
            ...
            if is_success:
                ...
                if post_cmds:
                    ...
                    for p_addr, p_vals in post_cmds:
                        ota_state["logs"].append(f"▶️ [后置操作] 向站号 {slave_val} 地址 {p_addr} 写入: {p_vals}")
                        write_ok, write_res = modbus_comm.master_write_10(port, baud, slave_val, p_addr, p_vals)
                        if write_ok:
                ...
                ota_state["logs"].append(f"⚠️ 警告：{file_name} 发生错误，系统忽略此错误并继续...")
            ...
            if current_run < total_runs:
                ...
                for sec in range(wait_seconds, 0, -1):
                    if not ota_state["is_running"]: return
    ...
    if fail_count == 0:
        ...
        ota_state["current_msg"] = f"批量执行结束！✅ 成功: {success_count} 次，❌ 失败: {fail_count} 次。请查阅日志。"
        ota_state["msg_status"] = "warning"
...
def parse_multi_cmds(addr_str, val_str, task_name, cmd_type):
    ...
    if bool(addr_str) != bool(val_str):
        raise ValueError(f"❌ 卡片【{task_name}】的【{cmd_type}】配置不完整！")
    ...
    if len(a_parts) != len(v_parts):
        raise ValueError(f"❌ 卡片【{task_name}】的【{cmd_type}】地址数与数值组数不匹配！")
    cmds = []
    try:
...
def render():
    ...
    st.markdown("支持**可视化添加多任务卡片**与**循环压测**。具备全局断线重连能力，**关闭浏览器依然不影响后台刷机进度**。")
    st.divider()

    # 🌟 核心绑定：永远指向全局独立内存
    ota_state = GLOBAL_OTA_STATE

    if ota_state["is_running"] or ota_state["result"] is not None:
        st.subheader("🔄 升级任务监视器")
        if ota_state["is_running"]:
            st.warning("⚠️ 批量升级任务正在后台排队执行中。您可以随意关闭网页，稍后重新打开即可无缝接续查看进度！")
        ...
        for i in range(st.session_state.ota_task_count):
            with st.container(border=True):
                ...
                with tc2: offset_val = st.text_input("文件偏移地址", value="0", key=f"offset_{i}")
                ...
                with st.expander("🛠️ 高级：升级前/后附加操作 (支持多指令分号隔开)", expanded=False):
                    ...
                    with ec1: pre_addr = st.text_input("前置-触发地址", value="", key=f"pre_addr_{i}")
                    with ec2: pre_val = st.text_input("前置-写入数值", value="", key=f"pre_val_{i}")
                    ec3, ec4 = st.columns(2)
                    with ec3: post_addr = st.text_input("后置-触发地址", value="", key=f"post_addr_{i}")
                    with ec4: post_val = st.text_input("后置-写入数值", value="", key=f"post_val_{i}")
        ...
        with c6:
            loop_count = st.number_input("♻️ 队列循环次数", min_value=1, value=1)
        with c7:
            wait_minutes = st.number_input("⏱️ 冷却等待(分钟)", min_value=0.0, value=1.0, step=0.5)
        with c8:
